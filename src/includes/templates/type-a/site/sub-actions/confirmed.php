...
echo str_replace('%%title%%', __('Confirmation', 'comment-mail'), $site_header); ?>
    ...
    <div class="confirm">
        ...
        <?php else : // Confirmed successfully. ?>
            ...
            <h4>
                <?php if ($sub_comment) : // Subscribing to a specific comment? ?>
                    ...
                    <?php if ($subscribed_to_own_comment) : ?>
                        <?php echo sprintf(__('You\'ll be notified about replies to <a href="%1$s">your comment</a>; on:', 'comment-mail'), esc_attr($sub_comment_url)); ?>
                    <?php else : // The comment was not authored by this subscriber; i.e. it's not their own. ?>
                        <?php echo sprintf(__('You\'ll be notified about replies to <a href="%1$s">comment ID #%2$s</a>; on:', 'comment-mail'), esc_attr($sub_comment_url), esc_html($sub_comment->comment_ID)); ?>
                    <?php endif; ?>

                <?php else : // All comments/replies on this post. ?>
                    <?php echo __('You\'ll be notified about all comments/replies to:', 'comment-mail'); ?>
                <?php endif; ?>
            </h4>
            ...
            <p>
                <?php echo sprintf(__('Your email address is: &lt;<code>%1$s</code>&gt;;', 'comment-mail'), esc_html($sub->email)); ?><br />
                <?php echo sprintf(__('You chose delivery option: <code>%1$s</code>;', 'comment-mail'), esc_html($sub_deliver_label)); ?>
                <?php
                switch ($sub->deliver) {
                    case 'asap': // Instant notifications?
                        echo __('each email notification will be delivered to you instantly.', 'comment-mail');
                        break; // Break switch handler.

                    case 'hourly': // As a digest?
                    case 'daily':
                    case 'weekly':
                        echo __('notifications will be delivered as a digest.', 'comment-mail');
                        break; // Break switch handler.
                } ?>
            </p>
