        ...
        <?php else : // Unsubscribed successfully. ?>
            ...
            <h4>
                <?php if ($sub_comment) : // Unsubscribed from a specific comment? ?>
                    ...
                    <?php if ($subscribed_to_own_comment) : ?>
                        <?php echo sprintf(__('You\'ll no longer be notified about replies to <a href="%1$s">your comment</a>; on:', 'comment-mail'), esc_html($sub_comment_url)); ?>
                    <?php else : // The comment was not authored by this subscriber; i.e. it's not their own. ?>
                        <?php echo sprintf(__('You\'ll no longer be notified about replies to <a href="%1$s">comment ID #%2$s</a>; on:', 'comment-mail'), esc_html($sub_comment_url), esc_html($sub_comment->comment_ID)); ?>
                    <?php endif; ?>

                <?php else : // All comments/replies on this post. ?>
                    <?php echo __('You\'ll no longer be notified about comments/replies to:', 'comment-mail'); ?>
                <?php endif; ?>
            </h4>
            ...
            <h4>
                ...
                <?php else : // Unsubscribing from all comments/replies; fallback w/ just the `$sub->post_id`. ?>
                    <?php echo sprintf(__('Post ID #<code>%1$s</code>', 'comment-mail'), esc_html($sub->post_id)); ?>
                <?php endif; ?>
            </h4>
