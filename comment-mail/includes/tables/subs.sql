CREATE TABLE IF NOT EXISTS `%%prefix%%subs` (
  ...
  `key` varchar(16) COLLATE utf8_unicode_ci NOT NULL COMMENT 'A unique, unguessable, case insensitive key for each row.',
  ...
  `fname` varchar(50) COLLATE utf8_unicode_ci NOT NULL COMMENT 'First name. If empty, and we have a `user_id`, use value from `wp_usermeta` table.',
  `lname` varchar(100) COLLATE utf8_unicode_ci NOT NULL COMMENT 'Last name. If empty, and we have a `user_id`, use value from `wp_usermeta` table.',
  `email` varchar(100) COLLATE utf8_unicode_ci NOT NULL COMMENT 'Email address. If empty, and we have a `user_id`, use value from `wp_users` table.',
