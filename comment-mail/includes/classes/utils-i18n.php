	...
	if(!class_exists('\\'.__NAMESPACE__.'\\utils_i18n'))
	{
		...
		class utils_i18n extends abs_base
		{
			...
			public function status_label($status, $transform = 'lower')
			{
				...
				{
					case 'approve':
						$i18n = __('approved', $this->plugin->text_domain); break;
					case 'hold':
						$i18n = __('pending', $this->plugin->text_domain); break;
					case 'trash':
						$i18n = __('trashed', $this->plugin->text_domain); break;
					case 'spam':
						$i18n = __('spammy', $this->plugin->text_domain); break;
					case 'delete':
						$i18n = __('deleted', $this->plugin->text_domain); break;
					case 'open':
						$i18n = __('open', $this->plugin->text_domain); break;
					case 'closed':
						$i18n = __('closed', $this->plugin->text_domain); break;
					case 'unconfirmed':
						$i18n = __('unconfirmed', $this->plugin->text_domain); break;
					case 'subscribed':
						$i18n = __('subscribed', $this->plugin->text_domain); break;
					case 'suspended':
						$i18n = __('suspended', $this->plugin->text_domain); break;
					case 'trashed':
						$i18n = __('trashed', $this->plugin->text_domain); break;
					default: // Default case handler.
 			      if($status) // Only if it's not empty.
 			        $i18n = __(rtrim($status, 'ed').'ed', $this->plugin->text_domain); break;
				}
				if(ctype_alnum($i18n)) switch($transform)
			  {
			    case 'lower':
			...
			public function event_label($event, $transform = 'lower')
			{
				...
				{
					case 'inserted':
						$i18n = __('inserted', $this->plugin->text_domain); break;
					case 'updated':
						$i18n = __('updated', $this->plugin->text_domain); break;
					case 'overwritten':
						$i18n = __('overwritten', $this->plugin->text_domain); break;
					case 'purged':
						$i18n = __('purged', $this->plugin->text_domain); break;
					case 'cleaned':
						$i18n = __('cleaned', $this->plugin->text_domain); break;
					case 'deleted':
						$i18n = __('deleted', $this->plugin->text_domain); break;
					case 'invalidated':
						$i18n = __('invalidated', $this->plugin->text_domain); break;
					case 'notified':
						$i18n = __('notified', $this->plugin->text_domain); break;
					default: // Default case handler.
			...
			public function deliver_label($deliver, $transform = 'lower')
			{
				...
				{
					case 'asap':
						$i18n = __('asap', $this->plugin->text_domain); break;
					case 'hourly':
						$i18n = __('hourly', $this->plugin->text_domain); break;
					case 'daily':
						$i18n = __('daily', $this->plugin->text_domain); break;
					case 'weekly':
						$i18n = __('weekly', $this->plugin->text_domain); break;
					default: // Default case handler.
			...
			public function sub_type_label($sub_type, $transform = 'lower')
			{
				...
				{
					case 'comments':
						$i18n = __('all comments', $this->plugin->text_domain); break;
					case 'comment':
						$i18n = __('replies only', $this->plugin->text_domain); break;
					default: // Default case handler.
  		      if($action) // Only if it's not empty.
			        $i18n = __(rtrim($action, 'ed').'ed', $this->plugin->text_domain);
			      break;
